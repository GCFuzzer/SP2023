...
public class SnoopInstructionMethodAdapter extends MethodVisitor implements Opcodes {
  ...
  private void addConditionalJumpInstrumentation(int opcode, Label finalBranchTarget) {
    ...
    addBipushInsn(mv, instrumentationState.incAndGetId());
    addBipushInsn(mv, lastLineNumber);
    mv.visitMethodInsn(INVOKESTATIC, Config.instance.analysisClass, "BLOCK_BEGIN",
            "(II)V", false);
  ...
  public void visitJumpInsn(int opcode, Label label) {
    ...
    switch (opcode) {
      ...
      case IFNULL:
      case IFNONNULL:
        addConditionalJumpInstrumentation(opcode, label);
        break;
      case GOTO:
      case JSR:
  ...
  public void visitTableSwitchInsn(int min, int max, Label dflt, Label... labels) {
    Label[] labels1=new Label[labels.length];
    ...
    mv.visitTableSwitchInsn(min, max, defaultLabel, labels1);
  ...
  public void visitLookupSwitchInsn(Label dflt, int[] keys, Label[] labels) {
    ...
    mv.visitLookupSwitchInsn(defaultLabel,keys,labels1);
