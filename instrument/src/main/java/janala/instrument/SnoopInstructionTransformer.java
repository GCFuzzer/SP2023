...
public class SnoopInstructionTransformer implements ClassFileTransformer {
  ...
  private static String[] banned = {"[", "java/lang", "janala", "org/objectweb/asm", "sun", "jdk", "java/util/function", "java/util/regex"};
...
/*    if (cname.equals("java/lang/reflect/Method")) {
      return false;
    }*/
    for (String e : banned) {
  ...
  synchronized public byte[] transform(ClassLoader loader, String cname, Class<?> classBeingRedefined,
    ...
    if (toInstrument) {
      ...
      try {
        ...
        ret = cw.toByteArray();
        Files.write(new File("./bytecode/"+cname.replace("/",".") + ".class").toPath(),ret);
      } catch (Throwable e) {
